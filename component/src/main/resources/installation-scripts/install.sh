...
then
    if [ ! -e temp ]
    then mkdir temp
    cp $ballerina_lib_location/in2-$package_name-package-$version.jar temp/
    fi
fi

cp dependencies/in2-$package_name-package-$version.jar $ballerina_lib_location
...
if [ $? -ne 0 ]; then
    ...
    if [ -e temp/in2-$package_name-package-$version.jar ]
    then cp temp/in2-$package_name-package-$version.jar $ballerina_lib_location/
    rm -r temp
    else rm $ballerina_lib_location/in2-$package_name-package-$version.jar
    fi
    echo "You can install the package by manually copying"
    echo 1. "dependencies/in2-$package_name-package-$version.jar to $ballerina_lib_location"
    echo 2. "Contents of balo directory to $ballerina_balo_location"
